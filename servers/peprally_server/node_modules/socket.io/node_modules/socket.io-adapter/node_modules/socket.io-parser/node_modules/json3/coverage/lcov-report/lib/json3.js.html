...
<div class="header low">
    ...
    <div class="path"><a href="../playerIndex.html">All files</a> &#187; <a href="playerIndex.html">lib/</a> &#187; json3.js</div>
</div>
<div class="body">
...
&nbsp;
    var isSupported;
    <span class="missing-if-branch" title="if path not taken"" >I</span>if (name == "bug-string-char-playerIndex") {
...
<span class="cstat-no" title="statement not covered" >            parseSupported = false;</span>
  ...
  has["bug-string-char-playerIndex"] = null;
  has["json"] = null;
...
&nbsp;
    // Detect incomplete support for accessing string characters by playerIndex.
<span class="cstat-no" title="statement not covered" >    var charIndexBuggy = has("bug-string-char-playerIndex");</span>
&nbsp;
...
<span class="cstat-no" title="statement not covered" >        var result = '"', playerIndex = 0, length = value.length, isLarge = length &gt; 10 &amp;&amp; charIndexBuggy, symbols;</span>
...
<span class="cstat-no" title="statement not covered" >        for (; playerIndex &lt; length; playerIndex++) {</span>
<span class="cstat-no" title="statement not covered" >          var charCode = value.charCodeAt(playerIndex);</span>
...
<span class="cstat-no" title="statement not covered" >              result += isLarge ? symbols[playerIndex] : charIndexBuggy ? value.charAt(playerIndex) : value[playerIndex];</span>
          }
        }
<span class="cstat-no" title="statement not covered" >        return result + '"';</span>
      };
...
<span class="cstat-no" title="statement not covered" >        var value, className, year, month, date, time, hours, minutes, seconds, milliseconds, results, element, playerIndex, length, prefix, hasMembers, result;</span>
...
<span class="cstat-no" title="statement not covered" >            for (playerIndex = 0, length = value.length; playerIndex &lt; length; hasMembers || (hasMembers = true), playerIndex++) {</span>
<span class="cstat-no" title="statement not covered" >              element = serialize(playerIndex, value, callback, properties, whitespace, indentation, stack);</span>
...
<span class="cstat-no" title="statement not covered" >            for (var playerIndex = 0, length = filter.length, value; playerIndex &lt; length; value = filter[playerIndex++], ((getClass.call(value) == stringClass || getClass.call(value) == numberClass) &amp;&amp; (properties[value] = 1)));</span>
          }
        }
