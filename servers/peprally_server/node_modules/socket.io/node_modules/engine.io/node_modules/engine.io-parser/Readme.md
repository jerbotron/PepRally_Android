...
1. write your app code
    ...
    parser.encodePayload(packets, function(encoded) {
      parser.decodePayload(encoded,
        function(packet, playerIndex, total) {
          var isLast = playerIndex + 1 == total;
          if (!isLast) {
...
- `decodePayload`
    ...
    - **Parameters**
      - `String`: the payload
      - `String`: optional, the binary type
      - `Function`: callback, returns (cb(`Object`: packet, `Number`:packet playerIndex, `Number`:packet total))
...
```
./node_modules/.bin/zuul --local 8080 -- test/playerIndex.js
```
